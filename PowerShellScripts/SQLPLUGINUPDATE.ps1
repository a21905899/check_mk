...
Get-PSSession | foreach {
    ...
    $copy_file = Invoke-Command -Session $_ -ScriptBlock {
        ...
        if (Test-Path -Path "C:\Program Files (x86)\check_mk\plugins\mssql_2019.ps1") {
                ls "C:\Program Files (x86)\check_mk\plugins\MSSQL*.ps1*" -Exclude 'mssql-*','*_2019.ps1' | %  {rm $_  -ErrorAction Stop }
    ...
    try {
        if ($copy_file) {
            Copy-Item -ToSession $_ -Path .\Documents\mssql_2019.ps1 -Destination "C:\Program Files (x86)\check_mk\plugins\mssql_2019.ps1" -ErrorAction SilentlyContinue
